...
static void e_emlrt_marshallIn(const emlrtStack *sp, const mxArray *nullptr,
                               const char_T *identifier, snaData_t *y);
...
static const mxArray *emlrt_marshallOut(const snaData_t *u);
 ...
 */
static void f_emlrt_marshallIn(const emlrtStack *sp, const mxArray *u,
                               const emlrtMsgIdentifier *parentId, snaData_t *y)
{
...
void ekf10Init_atexit(void)
{
  ...
  ekf10Init_xil_terminate();
  ekf10Init_xil_shutdown();
  emlrtExitTimeCleanup(&emlrtContextGlobal);
}
...
/*
 * File trailer for _coder_ekf10Init_api.c
 *
 * [EOF]
 */
